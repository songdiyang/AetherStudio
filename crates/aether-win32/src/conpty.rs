...
mod tests {
    ...
    // ConPTY 测试串行化：CreatePseudoConsole/CreatePipe 在并行运行时可能互相干扰
    static CONPTY_TEST_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

    /// 诊断测试：验证子进程能否成功启动并保持运行。
    #[test]
    fn test_conpty_spawns_cmd_and_stays_alive() {
        let _lock = CONPTY_TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let cmd = r"C:\Windows\System32\cmd.exe";
        let result = ConPtySession::spawn(cmd, None, 80, 24);
        match &result {
            Ok((session, _read_handle)) => {
                std::thread::sleep(std::time::Duration::from_millis(500));
                let alive = session.is_alive();
                assert!(alive, "cmd.exe 在 500ms 内退出，启动方式可能有误");
                println!("✓ cmd.exe 启动成功并保持运行");
            }
            Err(e) => {
                panic!("spawn 失败: {}", e);
            }
    ...
    /// 诊断测试：验证启动子进程后能否读取到输出。
    #[test]
    fn test_conpty_reads_cmd_output() {
        let _lock = CONPTY_TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let cmd = r"C:\Windows\System32\cmd.exe";
        let (session, read_handle) = ConPtySession::spawn(cmd, None, 80, 24)
            .expect("spawn 失败");

        session.write_input(b"echo hello_conpty_test\r").unwrap();
        ...
        let output = String::from_utf8_lossy(&total);
        println!("输出 ({} bytes): {}", total.len(), output);

        assert!(session.is_alive(), "cmd.exe 应保持运行状态");
        assert!(!total.is_empty(), "应能读取到输出，但读取到 0 bytes");
    }
}
