...
mod tests {
    ...
    fn test_command_id_all_labels() {
        use CommandId::*;
        let ids = [
            None, FileNew, FileNewWindow, FileOpen, FileOpenFolder, FileCloseWorkspace,
            FileSave, FileSaveAs, FileExit, EditUndo, EditRedo, EditCut, EditCopy, EditPaste,
            EditFind, EditReplace, EditSelectAll, SelectAll, ViewToggleSidebar,
            ViewToggleActivityBar, ViewToggleStatusBar, ViewZoomIn, ViewZoomOut,
            GotoFile, GotoLine, RunStart, RunDebug, TerminalNew, SearchGlobal,
            AiFixDiagnostics, HelpAbout,
        ];
        for id in &ids {
