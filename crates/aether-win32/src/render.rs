...
impl EditorState {
    pub fn render(&mut self) {
        ...
        let cursor_moved =
            self.content.cursor_line != self.last_cursor_line || self.content.cursor_col != self.last_cursor_col;
        ...
        if !self.dirty_tracker.is_full_window() {
            match render_cmd {
                crate::dirty_rect::RenderCommand::EditorOnly => {
                    let line_height = self.text_renderer.line_height();
                    let editor_content_region = self.layout.editor_content_region(show_tab_bar);
                    let cursor_y = editor_content_region.y + self.content.cursor_line as f32 * line_height
        ...
        if use_clip {
            ...
            if !rects.is_empty() {
                let rect_tuples: Vec<(f32, f32, f32, f32)> = rects
                    ...
                    .collect();
                use_layer = self.render_ctx.push_multi_clip(&self.d2d_factory.factory(), &rect_tuples);
            }
        }
        ...
        unsafe {
            ...
            if !self.terminal_panel.running {
                ...
                let hint_format = self
                    ...
                    .unwrap();
                let hint_text: Vec<u16> = "按 Ctrl+` 启动终端"
                    .encode_utf16()
                    .chain(Some(0))
                    .collect();
                let hint_rect = D2D_RECT_F {
    ...
    fn render_ai_assistant_sidebar(
        ...
        unsafe {
            ...
            // ===== 模式切换按钮 (Ask / Edit / Agent) =====
            self.ai_panel.clear_hit_regions();
            let modes = [AiMode::Ask, AiMode::Edit, AiMode::Agent];
            let mode_btn_w = ((width - margin * 2.0 - 8.0) / 3.0).max(1.0);
            let mode_btn_h = 24.0f32;
            for (i, m) in modes.iter().enumerate() {
                let bx = x + margin + i as f32 * (mode_btn_w + 4.0);
                let by = cy;
                let is_active = self.ai_panel.mode == *m;
                let mode_color = if is_active {
                    color_f(0.0, 0.47, 0.83, 1.0)
                } else {
                    color_f(0.20, 0.20, 0.22, 1.0)
                };
                let mode_brush = match self.render_ctx.brush_cache.get_brush(target, &mode_color) {
                    Ok(b) => b,
                    Err(_) => continue,
                };
                let mrect = D2D_RECT_F {
                    left: bx,
                    top: by,
                    right: bx + mode_btn_w,
                    bottom: by + mode_btn_h,
                };
                target.FillRectangle(&mrect, &mode_brush);
                let label_wide: Vec<u16> = m.label().encode_utf16().chain(Some(0)).collect();
                let label_rect = D2D_RECT_F {
                    left: bx,
                    top: by + 4.0,
                    right: bx + mode_btn_w,
                    bottom: by + mode_btn_h - 2.0,
                };
                target.DrawText(
                    &label_wide,
                    &small_format,
                ...
                self.ai_panel
                    .mode_button_regions
                    .push((*m, bx, by, mode_btn_w, mode_btn_h));
            }
            cy += mode_btn_h + 8.0;
    ...
    fn render_ai_settings_fields(
        ...
        unsafe {
            ...
            // Temperature
            let temp_label: Vec<u16> = "Temperature (0.0-2.0)"
                .encode_utf16()
                .chain(Some(0))
                .collect();
    ...
    fn render_editor(
        ...
        unsafe {
            ...
            // P0-3: 光标 x 减去水平滚动偏移
            let cursor_x =
                x + line_number_width + 5.0 - self.content.scroll_x + cursor_char_col as f32 * char_width;
            let cursor_y = y + (self.content.cursor_line.saturating_sub(start_line)) as f32 * line_height
                - (self.content.scroll_y % line_height);
    ...
    fn render_inline_completion(
        ...
        unsafe {
            ...
            let ghost_x =
                x + line_number_width + 5.0 - self.content.scroll_x + cursor_char_col as f32 * char_width;
            let ghost_y = y + (self.content.cursor_line.saturating_sub(start_line)) as f32 * line_height
                - (self.content.scroll_y % line_height);
    ...
    fn render_title_bar(
        ...
        unsafe {
            ...
            let user_bg = if user_btn_hover { &hover_min_bg } else { &default_bg };
            let user_bg_brush = self.render_ctx.brush_cache.get_brush(target, user_bg).unwrap();
            ...
            for (i, item) in self.explorer_context_menu.items.iter().enumerate() {
                if item.is_separator() {
                    ...
                    let label_wide: Vec<u16> =
                        item.label().encode_utf16().chain(Some(0)).collect();
                    let label_rect = D2D_RECT_F {
                        left: menu_x + 16.0,
                        top: current_y,
                        right: menu_x + menu_width - 16.0,
    ...
    fn render_tab_context_menu(
        ...
        unsafe {
            ...
            let normal_text_brush =
                match self.render_ctx.brush_cache.get_brush(target, &normal_text_color) {
                    Ok(b) => b,
                    Err(_) => return,
                };
            let hover_text_color = color_f(1.0, 1.0, 1.0, 1.0);
            let hover_text_brush =
                match self.render_ctx.brush_cache.get_brush(target, &hover_text_color) {
                    Ok(b) => b,
                    Err(_) => return,
                };
            let disabled_text_color = color_f(120.0 / 255.0, 120.0 / 255.0, 120.0 / 255.0, 1.0);
            let disabled_text_brush =
                match self.render_ctx.brush_cache.get_brush(target, &disabled_text_color) {
                    Ok(b) => b,
                    Err(_) => return,
                };
            let hover_bg_color = color_f(
                80.0 / 255.0,
                120.0 / 255.0,
                200.0 / 255.0,
                200.0 / 255.0,
            ...
            let hover_bg_brush =
                match self.render_ctx.brush_cache.get_brush(target, &hover_bg_color) {
                    Ok(b) => b,
                    Err(_) => return,
                };
            ...
            for (i, item) in self.tab_context_menu.items.iter().enumerate() {
                if item.is_separator() {
                    ...
                    let sep_rect = D2D_RECT_F {
                        ...
                        right: menu_x + menu_width - 8.0,
                        bottom: current_y + (self.tab_context_menu.separator_height - 1.0) / 2.0
                            + 1.0,
                    };
                    target.FillRectangle(&sep_rect, &sep_brush);
            ...
            for (i, item) in self.activity_bar_context_menu.items.iter().enumerate() {
                if item.is_separator() {
                    ...
                    if item.checked {
                        ...
                        let cy = current_y;
                        // ✓ 由两段线段构成：下笔 → 底部 → 右上
                        let p0 = D2D_POINT_2F { x: cx, y: cy + 15.0 };
                        let p1 = D2D_POINT_2F { x: cx + 4.0, y: cy + 19.0 };
                        let p2 = D2D_POINT_2F { x: cx + 10.0, y: cy + 11.0 };
        ...
        for item in &menu_item.items {
            ...
            let shortcut_w = item
                ...
                .and_then(|s| {
                    self.render_ctx
                        .text_format_cache
                        .measure_text_width(s, SHORTCUT_FONT_SIZE, normal_weight)
                })
                .unwrap_or(0.0);
