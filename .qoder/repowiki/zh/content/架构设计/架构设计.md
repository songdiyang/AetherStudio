...
- 面板与布局：活动栏、标签页、命令面板、设置、搜索、AI 面板、终端等
...
Win->>Win : 消息循环/输入处理
Win->>Core : 文本编辑/词法分析
Core-->>Win : 文本/Token 结果
Win->>Render : 请求绘制主题/文本/颜色
Render->>D2D : 提交绘制命令
D2D-->>Win : 完成绘制
end
```
...
## 结论
本项目以 Cargo Workspace 为核心组织方式，清晰划分 UI、核心、渲染抽象与系统 API 层，结合 Piece Table 与 Direct2D 自绘渲染，实现了高性能、低延迟、可扩展的编辑器体验。事件驱动与异步 IO 保障了 UI 流畅性，模块化设计便于持续演进与功能扩展。
